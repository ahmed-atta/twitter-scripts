...
class User {
						...
						$screen_name) 
	{
        ...
        if (!empty($result)) {
          ...
          $query = $mysqli->query("UPDATE `users` SET `twitter_oauth_token` = '$twitter_otoken',`twitter_oauth_token_secret` = '$twitter_otoken_secret',`accessToken` = '$access_token_oauth_token',`accessTokenSecret` = '$access_token_oauth_token_secret' WHERE  oauth_uid = '$uid' and oauth_provider = '$oauth_provider'") or die($mysqli->error);
        } else {
            #user not present. Insert a new Record
            $query = $mysqli->query("INSERT INTO `users` (oauth_provider, oauth_uid, username,twitter_oauth_token,twitter_oauth_token_secret,accessToken,accessTokenSecret,screen_name)

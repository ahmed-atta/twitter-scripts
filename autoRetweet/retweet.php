...
if(isset($_POST['action'])){
		if(isset($_POST['all_accounts']) && $_POST['all_accounts'] == "TRUE") {
			$result = $mysqli->query("SELECT U.accessToken, U.accessTokenSecret  FROM `re_users` AS U ");
		} else if(isset($_POST['custom_accounts']) && !empty($_POST['custom_accounts'])){
			$result = $mysqli->query("SELECT U.accessToken, U.accessTokenSecret  FROM `re_users` AS U  ORDER BY RAND() LIMIT 0,".$_POST['custom_accounts']);
		} else {
			$limitEnd =  $retweetPaging;
			$result = $mysqli->query("SELECT U.accessToken, U.accessTokenSecret  FROM `re_users` AS U  LIMIT $limitStart,$limitEnd ");
		}
		if ($result) {
		/* fetch associative array */
			...
			echo"<br/><center><span class='label label-success'> تم عمل ريتويت/المفضلة للتغريده  </span></center>

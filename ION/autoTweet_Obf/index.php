 ...
 * -------------------------------------*/
 @session_start();  if(isset($_POST['login'])){  require './config.php';  $username = mysqli_real_escape_string ( $R0DADB0D8CA9669A3F9F73A406CB0A0FC , $_POST['username'] );  $password = mysqli_real_escape_string ( $R0DADB0D8CA9669A3F9F73A406CB0A0FC , $_POST['password']);   $R4EEB713E57BBAAF1217CF39632604473['username'] = $R76DC4C449E8F2CF53739F99E73D8A052;  $R4EEB713E57BBAAF1217CF39632604473['password'] = $RD7F4A57E4625789A38C3C3DC44E8355B;  $R4EEB713E57BBAAF1217CF39632604473['id'] = 1;  if($username == $R4EEB713E57BBAAF1217CF39632604473['username'] && $password == $R4EEB713E57BBAAF1217CF39632604473['password']){    $_SESSION['tweet_admin'] = 1;   $_SESSION['username'] = $R4EEB713E57BBAAF1217CF39632604473['username'];   $_SESSION['admin_id'] = $R4EEB713E57BBAAF1217CF39632604473['id'];   header("Location:index.php");     }else {   $RDD95BBEC198B2C577F5B3E60A5984492 = " <div class='alert alert-danger'>البيانات غير صحيحه</div>  ";  } }   if(@$_SESSION['tweet_admin'] === 1){  require_once("header.php");  require './config.php';     if(isset($_GET['go'])){   switch($_GET['go']){    case 'logout':    {     session_destroy ();     @header('Location: index.php ');      echo "<meta http-equiv='refresh' content='0;URL=index.php'>";    } break;    default:     @header("Location: index.php");   }  }else {   @header("Location: accounts.php");   echo "<meta http-equiv='refresh' content='0;URL=accounts.php'>";  }    ?>
	</div>
</div>
<?php } else { ?>
<!DOCTYPE html>

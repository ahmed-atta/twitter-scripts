 ...
 * -------------------------------------*/
 session_start(); if($_SESSION['hashtag_admin'] === 1){  require './config.php';    if(isset($_GET['action']) && ($_GET['action'] =='del') && isset($_GET['id'])){   $R3584859062EA9ECFB39B93BFCEF8E869 =  $R0DADB0D8CA9669A3F9F73A406CB0A0FC->real_escape_string($_GET['id']);   $R0DADB0D8CA9669A3F9F73A406CB0A0FC->query("DELETE  FROM `users`  WHERE  id = '$R3584859062EA9ECFB39B93BFCEF8E869'");  }  if($R679E9B9234E2062F809DBD3325D37FB6 = $R0DADB0D8CA9669A3F9F73A406CB0A0FC->query("SELECT  *  FROM `users`")){   $RA4670B6CC8C17F77F7FE39EC1B8564DA = $R679E9B9234E2062F809DBD3325D37FB6->num_rows;  } ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<?php   } else {  header("Location: index.php");  echo "<meta http-equiv='refresh' content='0;URL=index.php'>"; } ?>
 ...
 * -------------------------------------*/ 
 ...
 $username = mysqli_real_escape_string ( $R0DADB0D8CA9669A3F9F73A406CB0A0FC , $_POST['username'] );  
 $password = mysqli_real_escape_string ( $R0DADB0D8CA9669A3F9F73A406CB0A0FC , $_POST['password']); 
	if (file_exists('./.dat')) {
		$fp = fopen('./.dat', "r");
		$contents = fread($fp, filesize("./.dat"));
		$pwd = explode(':',base64_decode($contents));
		$R4EEB713E57BBAAF1217CF39632604473['username'] = $pwd[0];
		$R4EEB713E57BBAAF1217CF39632604473['password'] = $pwd[1];
		fclose($fp);
	} else {
		$R4EEB713E57BBAAF1217CF39632604473['username'] = $R76DC4C449E8F2CF53739F99E73D8A052;  
		$R4EEB713E57BBAAF1217CF39632604473['password'] = $RD7F4A57E4625789A38C3C3DC44E8355B;  
	}

 $R4EEB713E57BBAAF1217CF39632604473['id'] = 1;   
 if($username == $R4EEB713E57BBAAF1217CF39632604473['username'] && $password == $R4EEB713E57BBAAF1217CF39632604473['password']){    $_SESSION['retweet_admin'] = 1;   $_SESSION['username'] = $R4EEB713E57BBAAF1217CF39632604473['username'];   $_SESSION['admin_id'] = $R4EEB713E57BBAAF1217CF39632604473['id'];     }else {   $RDD95BBEC198B2C577F5B3E60A5984492 = " <div class='alert alert-danger'>البيانات غير صحيحه</div>  ";  } }   if(@$_SESSION['retweet_admin'] === 1){  require_once('config.php');     if(isset($_GET['go'])){   switch($_GET['go']){    case 'logout':    {     session_destroy ();     @header('Location: index.php ');      echo "<meta http-equiv='refresh' content='0;URL=index.php'>";    } break;    default:     @header("Location: index.php");   }  }else {   @header("Location: accounts.php");   echo "<meta http-equiv='refresh' content='0;URL=accounts.php'>";  }    ?>
	</div>
</div>
<?php } else { ?>
<!DOCTYPE html>

			...
			/* fetch associative array */
				while ($user = $result->fetch_assoc()) {
					...
					if($_POST['action_type'] == "follow"){
						...
						$msg = "<div class='alert alert-danger'> تم عمل متابعة لهذا الحساب  </div><meta http-equiv='refresh' content='3'>";
					} else if($_POST['action_type'] == "unfollow"){
						if($isfollow->relationship->source->following){
							$twitter->post('friendships/destroy', array('screen_name' => $_POST['twitter_name']));  
						}
						$msg = "<div class='alert alert-danger'> تم إلغاء المتابعة لهذا الحساب  </div><meta http-equiv='refresh' content='3'>";
					}
				}
				$result->free(); 
		}

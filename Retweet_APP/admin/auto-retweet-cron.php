 ...
 */
 ...
 $rs_retweet = $mysqli->query("SELECT  *  FROM `re_auto`  WHERE admin_id = 1");
 ...
 if(mysqli_num_rows($rs_retweet) > 0){
	while($row = $rs_retweet->fetch_assoc()){
		switch($row['forT']){
			...
			break;
			default :
			$posts = $twitter->get('statuses/user_timeline', array('screen_name' => $row['twitter_account'],'count' => 3,'trim_user'=>1));
		}

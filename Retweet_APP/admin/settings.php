...
if(isset($_SESSION['retweet_admin']) && $_SESSION['retweet_admin'] == 1) {
	...
	if($osettings = $mysqli->query("SELECT * FROM re_options WHERE option_name ='settings' AND admin_id =".$_SESSION['admin_id'])){
	...
	if(isset($_POST['action'])){
		...
		if($osettings) {
			if($osettings->num_rows > 0 ){
				$mysqli->query("UPDATE `re_options` SET option_value ='$post_settings' WHERE option_name ='settings' AND admin_id =".$_SESSION['admin_id']);
			} else {
				$mysqli->query("INSERT INTO `re_options` (option_name,option_value,admin_id) VALUES ('settings','$post_settings','$admin_id');" );
			}
		}

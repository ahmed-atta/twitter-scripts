...
if(isset($_POST['action'])){
		#Insert a new Record
				...
				VALUES ('$twitter_name','$type',$admin_id,$counts,'$forTweet');");
				
		$msg= "<div class='alert alert-danger'>  سيتم عمل ريتويت/مفضله تلقائي لهذا الحساب  </div><meta http-equiv='refresh' content='1'>";
}
if(isset($_GET['action']) && $_GET['action'] == 'del'){
	$admin_id = $_SESSION['admin_id'];
	$mysqli->query("DELETE  FROM `re_auto`  WHERE admin_id='$admin_id' AND id=".$_GET['id']);
	$msg = "<div class='alert alert-danger'> تم حذف الحساب   </div><meta http-equiv='refresh' content='1;URL=auto-retweet.php'>";
}
//==============================================================//
?>
...
<h3> حساب الريتويت التلقائي </h3>
   <table class="table table-bordered">
    ...
    <tbody>
<?php  
if ($rs_retweet = $mysqli->query("SELECT  *  FROM `re_auto`  WHERE admin_id=".$_SESSION['admin_id'])) {
